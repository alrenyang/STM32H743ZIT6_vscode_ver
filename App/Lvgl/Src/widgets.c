...
static void Setting_window_close(ui_strobe_t * ui)
{
    if(!ui) return;
...
static uint32_t baud_from_idx(uint16_t idx)
{
    static const uint32_t tbl[] = { 9600, 19200, 38400, 57600, 115200, 230400 };
    if(idx >= (sizeof(tbl)/sizeof(tbl[0]))) return 115200;
    return tbl[idx];
}

static bool parse_octet(const char *s, uint8_t *out)
{
    if(!s || !*s) return false;
    int v = atoi(s);
...
static void setting_close_event_cb(lv_event_t * e)
{
    ui_strobe_t * ui = (ui_strobe_t *)lv_event_get_user_data(e);
    if(!ui) return;
...
/*-----------------------------------------------------------*/
static void Setting_mask_event_cb(lv_event_t * e)
{
    ui_strobe_t * ui = (ui_strobe_t *)lv_event_get_user_data(e);
    if(!ui) return;
...
static void Setting_window_open(ui_strobe_t * ui)
{
    if(!ui) return;
